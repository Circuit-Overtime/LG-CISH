...
%  Figures from ./evaluation/figures/. Conventions: passive voice,
...
\begin{algorithm}[t]
\caption{\textsc{Decode}: image sequence $\to$ message}
\label{alg:decode}
\begin{pseudo}
\item \kw{Input:} received images
  $\langle\tilde y_0,\dots,\tilde y_{T-1}\rangle$;
  codebook $\mathbf{C}$; key $\kappa$; flags \textit{perm}, $B$

\item \kw{for} $t=0$ \kw{to} $T-1$:\quad
  $\hat\imath_t\gets
  \arg\max_j s(e(\tilde y_t),\mathbf{C}_j)$
  \Cmt{\Eqn{eq:nnintro}}
...
\item \Ind
  $m'\gets P(N,B)$;\quad
  \kw{for each} block $i$:\quad
  $e_i\gets
  \bigl(
    \textsc{permDecode}(\hat\imath_{[iB:iB+B]},N)-\mu_i
  \bigr)\bmod m'$

\item \Ind
  $v\gets\textsc{fromBaseN}((e_0,\dots),m')$

\item \kw{else}\quad
  $v\gets\textsc{fromBaseN}(\hat\imath,N)$

\item
  $f\gets
  \textsc{stripSentinel}
  \bigl(\textsc{bytes}_{\mathrm{BE}}(v)\bigr)$
  \Cmt{inverse of \Eqn{eq:intmethod}}

\item \kw{return}
  \textsc{unwrap}$(f,\kappa)$
  \Cmt{verify CRC, decrypt, decompress}
\end{pseudo}
\end{algorithm}
...
% (Results body continues...)

% Note: the remainder of the results section was inlined unchanged from
% the original modular file; referencing figures now uses the updated
