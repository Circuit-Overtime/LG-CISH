% ===================================================================
%  LG-CISH - FULL, SELF-CONTAINED MANUSCRIPT.
%  Build: pdflatex Manuscript.tex (twice).
%  All text, algorithms, results, and references are included in this file.
%  All figure files are stored beside this file (no subfolders required).
% ===================================================================
...
\titleformat{\section}
  {\normalfont\sffamily\large\bfseries\raggedright}{\thesection}{0.65em}{}
\titleformat{\subsection}
  {\normalfont\sffamily\normalsize\bfseries\raggedright}{\thesubsection}{0.65em}{}
\titleformat{\subsubsection}
  {\normalfont\sffamily\normalsize\bfseries\raggedright}{\thesubsubsection}{0.65em}{}
...
{\raggedright\fontsize{18}{21}\selectfont\sffamily\bfseries
LG-CISH: Lossless Language-Guided Coverless Image Steganography via CLIP Semantic Hashing and Base-$N$ Positional Coding\par}
\vspace{7mm}
...
bit-exact reconstruction (BER $=0$) in twenty of twenty-one channel
...
\par\smallskip
\noindent\textbf{Keywords}\quad coverless steganography · information hiding · CLIP ·
...
an unmodified image contains no pixel-embedding artefact for an image-level
steganalyser to detect. Sequence-level selection and ordering may still provide
behavioural evidence, so both pixel-level detectability and cover plausibility are
evaluated in this study. The method is built around this distinction.
...
% ============================ 2. RELATED WORK ============================
\subsection{Background Literature}\label{sec:related}

\subsubsection{Traditional image steganography}
...
\subsubsection{Steganalysis and detection challenges}
...
\subsubsection{Coverless image steganography}
...
\subsubsection{Semantic representation and hash-based retrieval}
...
\subsubsection{Role of large language models in cover shaping}
Vision-capable large language models can assess whether candidate images share a
subject or theme and whether a sequence is contextually plausible. In LG-CISH this
capability is used only in an optional, offline alias-selection layer; it does not
encode the secret, determine the numerical payload, or participate in decoding.
The core channel remains deterministic base-$N$ or permutation coding, while CLIP
provides the robust image representation~\cite{radford2021}. This separation avoids
conflating semantic cover shaping with the lossless communication mechanism.
...
\begin{tabular}{@{}p{2.15cm}p{2.55cm}p{4.05cm}p{4.25cm}@{}}
\toprule
...
Motivated by these challenges, this work proposes LG-CISH. The plaintext is
compressed, integrity tagged, optionally encrypted, and converted into base-$N$ or
Lehmer-permutation digits. Those digits select unmodified images from a shared
codebook. At the receiver, CLIP nearest-neighbour matching recovers the image
indices before the numerical and cryptographic stages are inverted. An optional
vision-LLM layer selects semantically interchangeable aliases to improve the
plausibility of the transmitted sequence without changing its payload. This account
matches the implemented system and separates the core coding channel from the
optional language-guided cover-shaping component.
...
Equations~\eqref{eq:coverless}--\eqref{eq:chance} hold and detection is reduced to chance;
(ii) is lossless, so that Equations~\eqref{eq:berintro}--\eqref{eq:mutual} hold on a clean
channel; and (iii) recovers correctly after realistic transport, so that
Equation~\eqref{eq:marginintro} is satisfied across compression, noise, resizing, and
...
\begin{enumerate}
  ...
  \item Two interchangeable lossless codes are formalised --- base-$N$ positional
        coding at the $\log_2 N$ ceiling (Equation~\eqref{eq:capimg}) and Lehmer-permutation
        coding with a no-repeat guarantee (Equations~\eqref{eq:pnb}--\eqref{eq:capperm}) ---
...
The remainder of the paper follows the standard scientific-article structure
requested by the journal. Section~\ref{sec:method} describes the materials,
architecture, coding procedures, experimental protocol, baselines, and statistical
tests. Section~\ref{sec:results} reports the results together with comparison and
interpretation, and Section~\ref{sec:conclusion} states the principal conclusion.

% ###################################################################

\section{Materials and Methods}\label{sec:method}
% --- Formal model and analytical foundations ---
\subsection{Analytical framework}\label{sec:framework}
\subsubsection{The covert-communication model}\label{sec:model}
...
\subsubsection{Threat model and assumptions}\label{sec:threat}
...
retained, which is the regime in which Equation~\eqref{eq:marginintro} can be guaranteed.
\end{assumption}
...
\subsubsection{Embedding distortion and the payload it buys}\label{sec:distortion}
Any embedding of the form of Equation~\eqref{eq:emb} changes at least one pixel, so a
...
through Equation~\eqref{eq:pinsker}, with additional detectability. The proposal of this paper
...
\subsubsection{Statistical detectability and its fundamental limits}\label{sec:security}
...
\begin{equation}
  ...
  \label{eq:cachin}
\end{equation}
The security criterion in Equation~\eqref{eq:cachin} makes perfect security the zero-divergence
case. The operational meaning of Equation~\eqref{eq:kl} is exposed by hypothesis testing. For any
...
\begin{equation}
  ...
  \label{eq:perr}
\end{equation}
The detector error in Equation~\eqref{eq:perr} is bounded below, through Pinsker's inequality,
by the divergence
in Equation~\eqref{eq:kl}:
\begin{equation}
  P_E \;\ge\; \frac{1}{2}-\sqrt{\tfrac{1}{2}\,D\!\left(P_C\|P_S\right)} .
  \label{eq:pinsker}
\end{equation}
Two consequences are read directly from Equation~\eqref{eq:pinsker}. First, when
...
(Equation~\eqref{eq:psnr}), and even small distortion is increasingly detectable
(Equations~\eqref{eq:pinsker}--\eqref{eq:sqrtlaw}).

\subsubsection{Detector performance and the cost of a single error}\label{sec:detector}
...
\begin{equation}
  ...
  \label{eq:deflection}
\end{equation}
According to Equation~\eqref{eq:deflection}, a larger $d^2$ means an easier detection problem;
...
the chance line of Equation~\eqref{eq:chance}.
...
\subsubsection{The coverless principle: removing the footprint}\label{sec:coverless}
...
\begin{equation}
  ...
  P_S=P_C &\;\Longrightarrow\; D\!\left(P_C\|P_S\right)=0 .
  \end{aligned}
  \label{eq:coverless}
\end{equation}
Substituting Equation~\eqref{eq:coverless} into Equation~\eqref{eq:pinsker} yields the central promise of
the paradigm,
\begin{equation}
  P_E \;\ge\; \tfrac{1}{2},
  \label{eq:chance}
...
three paradigms are contrasted in Table~\ref{tab:paradigms}.
...
then a genuine natural image and Equation~\eqref{eq:coverless} holds exactly. The mapping
...
\subsubsection{The mapping formulation}\label{sec:mapping}
...
Equation~\eqref{eq:keyspaceintro}. Second, the map is \emph{positional}, so that the same image
...
\subsubsection{The channel and the recovery condition}\label{sec:channel}
...
then Equation~\eqref{eq:marginintro} is guaranteed to hold and every index is recovered
...
The consequence of Equation~\eqref{eq:marginintro} for a whole message is multiplicative. If
...
range of the channel, which drives Equation~\eqref{eq:allcorrect} towards one. Second, for
degradations beyond that range the integrity check of Equation~\eqref{eq:crc} converts the
...
\subsubsection{Semantic hashing with a vision--language model}\label{sec:clip}
...
\begin{equation}
  ...
  \label{eq:embedintro}
\end{equation}
so that the similarity in Equation~\eqref{eq:nnintro} is the cosine (inner) product
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:infonce}
\end{equation}
with an analogous text-to-image term. Minimising Equation~\eqref{eq:infonce} over a very large
...
mechanism that keeps the decoding margin of Equation~\eqref{eq:marginintro} positive in
practice.

\subsubsection{Capacity of the image-sequence channel}\label{sec:capacity}
Two lossless codes are developed in this paper. The first, base-$N$ positional
coding, attains the per-image ceiling of Equation~\eqref{eq:capimg} but permits an image to
...
\begin{equation}
  ...
  \label{eq:capperm}
\end{equation}
Stirling's approximation gives $\log_2 N!\approx N\log_2(N/e)$. For a full
block, $B=N$ and $C_{\text{perm}}\approx\log_2(N/e)$, which is below the
base-$N$ ceiling of Equation~\eqref{eq:capimg} by roughly $\log_2 e\approx 1.443$ bits per
image; smaller blocks recover most of the loss. Thus Equation~\eqref{eq:capimg} and
Equation~\eqref{eq:capperm} bound a controllable trade-off between raw capacity and cover
plausibility, which is one of the design levers of the method.

\subsubsection{The lossless requirement and integrity}\label{sec:lossless}
...
indices are recovered ($\gamma_t>0\ \forall t$, Equation~\eqref{eq:marginintro}). For the
...
\subsubsection{Source coding, expected length, and a worked example}\label{sec:worked}
...
of Equation~\eqref{eq:zlib} is used precisely to push the encoded size
towards this floor before any image is chosen. Once the framed payload occupies
$|f|$ bytes, the base-$N$ code of Equation~\eqref{eq:capimg} produces an expected sequence
length
\begin{equation}
...
A concrete illustration is given in Table~\ref{tab:worked} for the codebook used in this
work ($N=40$, $\log_2 N\approx 5.322$ bits per image). It is emphasised that the
figures in Table~\ref{tab:worked} are dominated, for short messages, by the fixed
...
\begin{table*}[t]
\centering
\caption{Worked example of Equation~\eqref{eq:explen} for the $N=40$ codebook. The framed size
...
% ###################################################################
\subsection{Study materials and experimental design}\label{sec:dataset}
The study used natural images drawn from three established image-processing
benchmarks: UCID, the Kodak Lossless True Color Image Suite, and the USC-SIPI
Image Database~\cite{ucid,kodak,uscsipi}. These sources were pooled to provide
heterogeneous scenes without introducing synthetic compression artefacts. Images
were normalised to a $512\times512$ RGB canvas, embedded with CLIP ViT-B/32, and
greedily filtered using the separation rule in Equation~\eqref{eq:sep}. The final
codebook contained $N=40$ images. Table~\ref{tab:datasets} identifies the source
materials, and Table~\ref{tab:setup} records the configuration needed to reproduce
the experiments.

\begin{table*}[t]
\centering
\caption{Image sources used to construct the LG-CISH codebook.}\label{tab:datasets}
\begin{tabular}{@{}lll@{}}
\toprule
Source & Character & Role \\
\midrule
UCID & $\sim$1338 uncompressed colour images & primary natural-scene pool \\
Kodak & 24 reference photographs & high-quality reference scenes \\
USC-SIPI & standard test images & canonical textures and portraits \\
\bottomrule
\end{tabular}
\end{table*}
...
Parameter & Value \\
\midrule
Codebook & UCID + Kodak + USC-SIPI; $N=40$ \\
Image size & $512\times512$ RGB \\
Semantic encoder & CLIP ViT-B/32; $d=512$ \\
Coding & base-$N$ or distinct-image permutation \\
Separation threshold & $\tau=0.85$ \\
Encryption/integrity/compression & AES-256-CBC/CRC-32/zlib \\
Optional cover shaping & vision-LLM alias verification \\
...
Figure~\ref{fig:flow} and is referred to throughout the encoding, decoding, and evaluation
...
Equation~\eqref{eq:embedintro}. So that the nearest-neighbour rule of Equation~\eqref{eq:nnintro} is
...
difficulty of maintaining separation. By Equation~\eqref{eq:capimg} the per-image capacity
...
below the threshold $\tau$ of Equation~\eqref{eq:sep}, because the embedding space becomes more
...
satisfies Equation~\eqref{eq:sep}. Under $\tau$-separation and transport stability
$\lVert e(T(c_j))-e(c_j)\rVert_2\le\delta$ with $2\delta<1-\tau$, the recovery rule
of Equation~\eqref{eq:nnintro} returns the correct index for every codebook image.
\end{definition}

% --- Inlined algorithms (Algorithms 1--5) ---

\begin{figure*}[t]\captionsetup{type=algorithm}
...
\item \textbf{Input:} candidate image set $\mathcal{D}$, encoder $\Phi$, threshold $\tau$
\item \textbf{Output:} codebook embeddings $\mathbf{C}$, image paths, size $N$
\item $E \gets [\,e(x)\ \text{for } x\in\mathcal{D}\,]$ \hfill{\footnotesize$\triangleright$\,\textit{normalise, Equation~\eqref{eq:embedintro}}}
\item $\mathcal{K}\gets\emptyset$
\item \textbf{for} $j=0$ \textbf{to} $|\mathcal{D}|-1$ \textbf{do}
\item \hspace*{1.3em} \textbf{if} $\max_{i\in\mathcal{K}} s(E_i,E_j)\le\tau$ \textbf{then}
           $\ \mathcal{K}\gets\mathcal{K}\cup\{j\}$ \hfill{\footnotesize$\triangleright$\,\textit{Equation~\eqref{eq:sep}}}
\item \textbf{end for}
\item \textbf{return} $\mathbf{C}\gets E[\mathcal{K}]$,\ \ paths$[\mathcal{K}]$,\ \ $N\gets|\mathcal{K}|$
\end{list}
\end{figure*}
...
\begin{list}{\footnotesize\arabic{algln}:}{%
    ...
    \setlength{\labelsep}{0.4em}\setlength{\itemsep}{1.5pt}%
    \setlength{\parsep}{0pt}\setlength{\topsep}{3pt}\small}
\item \textbf{function} \textsc{wrap}$(m,\kappa)$
\item \hspace*{1.3em} $p_0 \gets \textsc{zlib}(\textsc{utf8}(m))$ \hfill{\footnotesize$\triangleright$\,\textit{Equation~\eqref{eq:zlib}}}
\item \hspace*{1.3em} $t \gets p_0 \,\Vert\, \textsc{crc32}(p_0)$ \hfill{\footnotesize$\triangleright$\,\textit{append 4-byte CRC}}
\item \hspace*{1.3em} \textbf{if} $\kappa$ given \textbf{then} $IV\gets\textsc{rand}(16)$;\ \ \textbf{return} $IV \,\Vert\, \textsc{aes-cbc}_\kappa(\textsc{pad}(t),IV)$
\item \hspace*{1.3em} \textbf{else} \textbf{return} $t$ \hfill{\footnotesize$\triangleright$\,\textit{CRC-only mode}}
\item \textbf{function} \textsc{unwrap}$(f,\kappa)$
\item \hspace*{1.3em} \textbf{if} $\kappa$ given \textbf{then} $t\gets\textsc{unpad}(\textsc{aes-cbc}^{-1}_\kappa(f_{[16:]},f_{[:16]}))$ \textbf{else} $t\gets f$
\item \hspace*{1.3em} split $t=p_0\,\Vert\, c$;\ \ \textbf{if} $c\neq\textsc{crc32}(p_0)$ \textbf{then} \textbf{reject}
\item \hspace*{1.3em} \textbf{return} $p_0$
\end{list}
\end{figure*}
...
\begin{list}{\footnotesize\arabic{algln}:}{%
    ...
    \setlength{\labelsep}{0.4em}\setlength{\itemsep}{1.5pt}%
    \setlength{\parsep}{0pt}\setlength{\topsep}{3pt}\small}
\item \textbf{function} \textsc{permEncode}$(u,N,B)$ \hfill{\footnotesize$\triangleright$\,\textit{integer $\to$ $B$ distinct indices}}
\item \hspace*{1.3em} \textbf{for} $t=B-1$ \textbf{down to} $0$:\ \ $r_t\gets u\bmod(N-t)$;\ \ $u\gets\lfloor u/(N-t)\rfloor$
\item \hspace*{1.3em} $A\gets[0,1,\dots,N-1]$;\ \ \textbf{return} $[\,A.\text{pop}(r_t)\ \text{for}\ t=0\dots B-1\,]$
\item \textbf{function} \textsc{permDecode}$(\langle a_0,\dots,a_{B-1}\rangle,N)$ \hfill{\footnotesize$\triangleright$\,\textit{indices $\to$ integer}}
\item \hspace*{1.3em} $A\gets[0,\dots,N-1]$;\ \ $u\gets0$
\item \hspace*{1.3em} \textbf{for} $t=0$ \textbf{to} $B-1$:\ \ $r\gets A.\text{index}(a_t)$;\ \ $u\gets u(N-t)+r$;\ \ $A.\text{pop}(r)$
\item \hspace*{1.3em} \textbf{return} $u$ \hfill{\footnotesize$\triangleright$\,\textit{Equation~\eqref{eq:lehmermethod}}}
\end{list}
\end{figure*}
...
\begin{list}{\footnotesize\arabic{algln}:}{%
    ...
    \setlength{\labelsep}{0.4em}\setlength{\itemsep}{1.5pt}%
    \setlength{\parsep}{0pt}\setlength{\topsep}{3pt}\small}
\item \textbf{Input:} message $m$; codebook $\mathcal{C}$ (size $N$); key $\kappa$;
      flags \textit{perm}, block $B$, \textit{alias}
\item $f\gets\textsc{wrap}(m,\kappa)$ \hfill{\footnotesize$\triangleright$\,\textit{Algorithm~\ref{alg:wrap}}}
\item $v\gets\textsc{int}_{\mathrm{BE}}(\mathtt{0x01}\,\Vert\, f)$ \hfill{\footnotesize$\triangleright$\,\textit{Equation~\eqref{eq:intmethod}}}
\item \textbf{if} \textit{perm} \textbf{then}
\item \hspace*{1.3em} $m'\gets P(N,B)$;\ \ $(e_0,\dots)\gets\textsc{toBaseN}(v,m')$
\item \hspace*{1.3em} $\text{idx}\gets\textsc{concat}_i\ \textsc{permEncode}\big((e_i+\mu_i)\bmod m',N,B\big)$ \hfill{\footnotesize$\triangleright$\,\textit{Equation~\eqref{eq:whitenmethod}}}
\item \textbf{else}\ \ $\text{idx}\gets\textsc{toBaseN}(v,N)$ \hfill{\footnotesize$\triangleright$\,\textit{Equation~\eqref{eq:basenmethod}}}
\item \textbf{if} \textit{alias} \textbf{then} $S\gets[\,\textsc{chooseAlias}(j)\ \text{for}\ j\in\text{idx}\,]$
      \textbf{else} $S\gets[\,c_j\ \text{for}\ j\in\text{idx}\,]$
\item \textbf{return} image sequence $S$
\end{list}
\end{figure*}
...
\begin{list}{\footnotesize\arabic{algln}:}{%
    ...
    \setlength{\labelsep}{0.4em}\setlength{\itemsep}{1.5pt}%
    \setlength{\parsep}{0pt}\setlength{\topsep}{3pt}\small}
\item \textbf{Input:} received images $\langle\tilde y_0,\dots,\tilde y_{T-1}\rangle$;
      codebook $\mathbf{C}$; key $\kappa$; flags \textit{perm}, $B$
\item \textbf{for} $t=0$ \textbf{to} $T-1$:\ \ $\hat\imath_t\gets\arg\max_j s(e(\tilde y_t),\mathbf{C}_j)$ \hfill{\footnotesize$\triangleright$\,\textit{Equation~\eqref{eq:nnintro}}}
\item \textbf{if} \textit{perm} \textbf{then}
\item \hspace*{1.3em} $m'\gets P(N,B)$;\ \ \textbf{for each} block $i$:\ \ $e_i\gets(\textsc{permDecode}(\hat\imath_{[iB:iB+B]},N)-\mu_i)\bmod m'$
\item \hspace*{1.3em} $v\gets\textsc{fromBaseN}((e_0,\dots),m')$
\item \textbf{else}\ \ $v\gets\textsc{fromBaseN}(\hat\imath,N)$
\item $f\gets\textsc{stripSentinel}(\textsc{bytes}_{\mathrm{BE}}(v))$ \hfill{\footnotesize$\triangleright$\,\textit{inverse of Equation~\eqref{eq:intmethod}}}
\item \textbf{return} $\textsc{unwrap}(f,\kappa)$ \hfill{\footnotesize$\triangleright$\,\textit{verify CRC, decrypt, decompress}}
\end{list}
\end{figure*}
...
\begin{align}
  ...
  \label{eq:zlib}\\
  f   &= \textsc{aes}_{\kappa}\!\left(p_0 \,\Vert\, \textsc{crc32}(p_0)\right),
  \label{eq:wrapmethod}
\end{align}
...
$t_i = E_\kappa^{-1}(c_i)\oplus c_{i-1}$. The chaining in Equation~\eqref{eq:cbc} ensures that
...
the recomputed check disagrees, thereby realising the guarantee of Equation~\eqref{eq:crc}.
...
The framed bytes $f$ of Equation~\eqref{eq:wrapmethod} are turned into codebook indices by one
...
\begin{equation}
  v=\textsc{int}_{\mathrm{BE}}\!\left(\mathtt{0x01}\,\Vert\, f\right).
  \label{eq:intmethod}
\end{equation}
...
\begin{equation}
  \begin{aligned}
  v&=\sum_{t=0}^{L-1} d_t\,N^{t},\\
  d_t&\in\{0,\dots,N-1\},\qquad L=\lfloor\log_N v\rfloor+1 ,
  \end{aligned}
  \label{eq:basenmethod}
\end{equation}
and digit $d_t$ selects image $c_{d_t}$. Because Equation~\eqref{eq:basenmethod} is a
bijection, decoding simply reads the digits back; this channel attains the ceiling
of Equation~\eqref{eq:capimg}, at the cost of allowing an image to repeat. As a small
...
instead written in radix $m=P(N,B)$ (Equation~\eqref{eq:pnb}), giving block values
...
\begin{equation}
  \begin{aligned}
  \tilde e_i&=\left(e_i+\mu_i\right)\bmod m,\\
...
The mechanics of Equation~\eqref{eq:lehmermethod} are best seen on a small example. Consider a
...
Equation~\eqref{eq:lehmermethod} reconstructs $\tilde e=17$, from which the whitening of
Equation~\eqref{eq:whitenmethod} is removed. The example makes concrete the two guarantees of
...
attains the ceiling of Equation~\eqref{eq:capimg}; permutation coding is preferred when the
...
regimes: a small block sacrifices little capacity relative to Equation~\eqref{eq:capimg} while
still forbidding repeats within the block, whereas the full-block case $B=N$ maximises
the no-repeat window at the largest capacity cost. In this way Equation~\eqref{eq:capperm}
...
message is framed by Equations~\eqref{eq:zlib}--\eqref{eq:wrapmethod}, mapped to the integer $v$
by Equation~\eqref{eq:intmethod}, converted to indices by either Equation~\eqref{eq:basenmethod} or the
whitened Lehmer code of Equation~\eqref{eq:whitenmethod}, and finally realised as images. When
...
CLIP and matched to the nearest codebook entry by Equation~\eqref{eq:nnintro}, which recovers
the transmitted index whenever the decoding margin of Equation~\eqref{eq:marginintro} is
...
(Equation~\eqref{eq:basenmethod} or the un-whitened Lehmer inverse of
Equations~\eqref{eq:lehmermethod}--\eqref{eq:whitenmethod}) to reconstruct the integer $v$, the
...
Equation~\eqref{eq:marginintro}, then $\textsc{Decode}(\textsc{Encode}(m))=m$ exactly and
$\mathrm{BER}=0$. This follows because each stage
--- Equation~\eqref{eq:zlib}, Equation~\eqref{eq:wrapmethod}, Equation~\eqref{eq:intmethod}, Equation~\eqref{eq:basenmethod},
and Equations~\eqref{eq:lehmermethod}--\eqref{eq:whitenmethod} --- is a bijection and is inverted
in reverse order.
\end{proposition}
...
dominated by big-integer arithmetic: the base-$N$ conversion of Equation~\eqref{eq:basenmethod}
...
\begin{equation}
  ...
  \label{eq:complexity}
\end{equation}
In Equation~\eqref{eq:complexity}, $C_\Phi$ is the cost of a single CLIP forward pass and
$d=512$ is the
...
Equation~\eqref{eq:marginintro} can turn negative at one or more positions, and by
Equation~\eqref{eq:allcorrect} the message-level success probability falls sharply. Crucially,
...
among aliases is invisible to the index-recovery rule of Equation~\eqref{eq:nnintro}, cover
...
Equation~\eqref{eq:nnintro}. It is stressed that aliases affect only the human plausibility of
the \emph{ordering} of the cover, not the distributional undetectability of each
individual image, which is already guaranteed by Equation~\eqref{eq:coverless}.

\subsection{Evaluation protocol and statistical analysis}\label{sec:protocol}
The evaluation measured reconstruction, robustness, capacity, distortion,
steganalysis resistance, and sequence plausibility. For a message encoded into
$L$ images, with intended and recovered indices $d_t$ and $\hat d_t$, and payload
bits $p_b$ and $\hat p_b$, the symbol error rate (SER) and bit error rate (BER) are
\begin{equation}
  \begin{aligned}
  \mathrm{SER}&=\frac{1}{L}\sum_{t=1}^{L}\mathbb{1}[\hat d_t\ne d_t],\\
  \mathrm{BER}&=\frac{1}{B}\sum_{b=1}^{B}\mathbb{1}[\hat p_b\ne p_b].
  \end{aligned}
  \label{eq:ber}
\end{equation}
Exact reconstruction requires $\mathrm{SER}=0$, which implies $\mathrm{BER}=0$
because both coding channels are bijective. Forty messages were tested for each
of 21 channel conditions: an unattacked control; JPEG quality 90, 70, 50, 30, and
20; Gaussian-noise standard deviation 5, 10, 20, and 30; salt-and-pepper density
0.01, 0.05, and 0.10; resizing to 50\%, 25\%, and 10\%; centre crops retaining
95\%, 90\%, 85\%, and 60\%; and WebP quality 80. Clean-channel summary statistics
used 50 independent trials.

Baselines comprised spatial LSB, transform-domain variants, a perceptual-hash
matcher, and representative retrieval-based coverless methods described in prior
work~\cite{chan2004,westfeld2001f5,subramanian,mandal,qin,mengReview}. A paired,
one-sided Wilcoxon signed-rank test compared CLIP and perceptual hashing across
seven crop strengths. A one-way ANOVA tested whether CLIP margin varied across
message-length groups. Statistical significance was assessed at $\alpha=0.05$.
Pixel-domain detection was evaluated with the chi-square statistic defined in
Equation~\eqref{eq:chi2}; sequence-level plausibility was evaluated separately by
a vision-LLM judge, because unmodified pixels alone do not guarantee natural image
...
transmitted sequence carry the secret. The full protocol and metrics are specified
in Section~\ref{sec:protocol}; this section reports and interprets the observations.

% ===================================================================
\subsection{Codebook characteristics}
The filtering procedure retained 40 mutually distinct images. This gives a
base-$40$ alphabet of $5.322$ bits per image while preserving an observed maximum
off-diagonal cosine similarity of $0.827$, equivalent to a separation margin of
$0.173$. Figure~\ref{fig:collage} shows a representative half of the codebook and
Figure~\ref{fig:heatmap} shows the complete pairwise similarity matrix. These
observations support the methodological choice of $\tau=0.85$: the retained images
are diverse enough for unambiguous clean-channel recovery, yet the codebook remains
large enough to provide more than five bits per transmitted image.
...
% ===================================================================
LG-CISH is compared in Table~\ref{tab:compare} with representative spatial,
transform-domain, and retrieval-based families. Prior reviews consistently describe
the capacity--imperceptibility--robustness trade-off of spatial and transform-domain
embedding~\cite{subramanian,kadhim,mandal}. Retrieval-based coverless methods avoid
pixel modification but remain constrained by database construction, indexing, and
feature robustness~\cite{qin,mengReview,neglected2022,end2endHash2023}. The measured
LG-CISH values are therefore interpreted against those established qualitative
properties rather than presented as a controlled reimplementation of every cited
method.

\begin{table*}[t]
\centering
\caption{Measured LG-CISH results contextualised against representative prior method families.}\label{tab:compare}
\begin{tabularx}{\textwidth}{@{}p{3.8cm}*{4}{>{\centering\arraybackslash}X}@{}}
\toprule
Method & Capacity & JPEG50 acc.\ (\%) & Detection (\%) & PSNR (dB) \\
\midrule
LSB (spatial)~\cite{chan2004} & high & 51.4 & 98 & 51.1 \\
DCT/F5 family~\cite{westfeld2001f5} & moderate & attack-dependent & detectable & finite \\
DWT--DCT family~\cite{subramanian,mandal} & moderate & attack-dependent & detectable & finite \\
Coverless retrieval~\cite{qin,mengReview} & low & feature-dependent & no embedding trace & $\infty$ \\
...
Because the transmitted images are bit-identical to their codebook originals, they
contain no embedding residue. The present chi-square experiment supports resistance
to that specific pixel-domain test, but it does not establish universal
undetectability: a specialised adversary could instead learn codebook membership,
selection frequencies, repeated identities, or implausible sequence order. This is
why sequence plausibility is evaluated separately and why testing against retrained
SRNet- and XuNet-type detectors remains future work. The combined keyspace is
$40!\cdot2^{256}\approx2^{415}$
...
\FloatBarrier
\clearpage
\subsection{Integrated discussion and limitations}\label{sec:discussion}
Taken together, the results show a deliberate exchange rather than a uniformly
superior steganographic channel. Conventional spatial and transform-domain methods
offer substantially greater payload density but modify pixels or coefficients and
must balance capacity, distortion, and detection risk~\cite{kadhim,subramanian,
mandal}. LG-CISH removes embedding distortion and retained exact reconstruction in
20 of 21 tested channel conditions, but its rate is only $5.322$ bits per image and
its transmitted sequences can be long. This database-bound capacity is consistent
with the limitations identified in reviews of retrieval-based coverless
steganography~\cite{qin,mengReview}.

The ablation result provides the clearest evidence for the choice of semantic
matching: both CLIP and perceptual hashing handled JPEG-$50$, whereas the
perceptual-hash variant failed under the tested crop sweep and CLIP retained exact
recovery. This extends work on learned or robust coverless hashing
~\cite{end2endHash2023,restoration2024} by using a pretrained vision--language
embedding without modifying the transmitted images. The comparison is nevertheless
limited to the stated datasets, codebook, attacks, and baselines; broader codebooks
and independently implemented contemporary methods are required before claiming
general superiority.

Finally, chance-level chi-square detection should not be read as proof that the
communication is undetectable in every threat model. The low plausibility score of
the diverse benchmark codebook demonstrates that a human or sequence-level detector
may exploit ordering and thematic inconsistency even when individual images are
unaltered. The optional alias layer improves the measured plausibility score, but
its dependence on a vision-LLM judge introduces model and prompt sensitivity. These
limitations delimit the claims supported by the current experiments and motivate
sequence-aware steganalysis, larger multi-source evaluations, and independent
replication.
...
\paragraph{Future work.}\label{sec:future}
Future studies should scale the codebook from $N{=}40$ to
$N{=}10^{3}$--$10^{4}$ while preserving its semantic separation margin, and should
improve sequence plausibility through theme-aware alias selection. Security should
also be evaluated with SRNet and XuNet retrained specifically on the proposed
channel. Combining distinct-image permutation with identity--ordering codes may
recover part of the capacity conceded by the coverless design. Finally, the method
should be extended to video and audio carriers and validated on public messaging
platforms with practical database-versioning and multi-party key-management
procedures.

\section*{Acknowledgements}

The authors acknowledge the use of benchmark images from the UCID database, the
Kodak Lossless True Color Image Suite, and the USC-SIPI Image Database. Images
from USC-SIPI Volume~3, including its Miscellaneous category, were used as
documented by Weber~\cite{uscsipi}.
...
The datasets, implementation, evaluation scripts, and the 40-image LG-CISH
codebook supporting the findings of this research are available in the LG-CISH
GitHub repository~\cite{lgcishGithub}.
...
\bibitem{lgcishGithub}
LG-CISH Authors,
``LG-CISH: Lossless generative coverless image steganography with semantic hashing,''
GitHub, 2026. [Online]. Available:
\url{https://github.com/Circuit-Overtime/LG-CISH}.
