...
publicly available benchmark suites, summarised in Table~\ref{tab:datasets}.
...
DIV2K was also considered, but the three chosen suites already provide sufficient
...
that underpins reliable decoding is visualised in Figure~\ref{fig:heatmap}.
...
signal-to-noise ratio and the structural similarity index between the cover $X$ and
...
\begin{equation}
  ...
  \label{eq:psnr_res}
\end{equation}
where a larger PSNR and an SSIM nearer one indicate less distortion. For every
...
imperceptibility (Eqn~\eqref{eq:infpsnr}) and high robustness
...
% ===================================================================
Capacity is the price paid for the imperceptibility of Eqn~\eqref{eq:infpsnr}.
...
\begin{equation}
  ...
  \label{eq:bpp}
\end{equation}
some five orders of magnitude below the $\approx 3$\,bpp of LSB. This large gap is
...
Eqn~\eqref{eq:infpsnr} and the robustness of Section~\ref{sec:robustness}.
...
through Eqn~\eqref{eq:ber}, the bit error rate is likewise zero. Over $N{=}50$
...
\begin{equation}
  ...
  \label{eq:chi2}
\end{equation}
where $F_{\chi^2}(\cdot;\nu)$ is the chi-square cumulative distribution with $\nu$
